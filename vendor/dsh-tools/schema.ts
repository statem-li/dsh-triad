...
// vendored: 值导入改指向本地内联副本（原因见 vendor/dsh-llm/index.ts 顶部说明）。
// 类型导入仍指向 DSH 包 —— 它们会在构建时被完全擦除，不参与运行时解析。
import { HarnessError } from '../dsh-llm/index.ts'
import type { ContentBlock } from '../dsh-llm/index.ts'
import type { JsonValue } from '@deepseek-ai/dsh-session'
import type { ToolDefinition, ToolExecution, ToolExecutionResult, ToolRunContext, ToolResult } from '@deepseek-ai/dsh-tools'
