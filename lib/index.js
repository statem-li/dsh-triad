...
var MemoryStore = class _MemoryStore {
  ...
  async appendLog(kind, line) {
    const { appendFile, stat: stat2, rename: rename5, readdir: readdir4, unlink: unlink2 } = await import("node:fs/promises");
    ...
    try {
      ...
      if (info.size >= maxBytes) {
        ...
        try {
          const names = (await readdir4(logDir)).filter((name2) => name2.startsWith(`${kind}.`) && name2.endsWith(".log")).sort();
...
async function consolidateScope(ctx, store, config, scope, trigger) {
  ...
  const llm = ctx.get("llm");
  if (llm === void 0) return empty;
  const route = await resolveRoute(ctx, CONSOLIDATE_AGENT);
  if (route === void 0) return empty;
  const controller = new AbortController();
...
function createMemoryInjector(store, config, logger) {
  ...
  const preStepListener = async (payload, next) => {
    ...
    try {
      ...
      const memoryMessage = createUserMessage3({
  ...
  async function runDailyCompile() {
    ...
    await store.replaceEntries((entries) => {
      ...
      if (survivor.length > config.entryLimit) {
        const overflow = survivor.filter((entry) => !entry.pinned && entry.deprecated !== true).sort((a, b) => a.importance - b.importance || a.updatedAt.localeCompare(b.updatedAt)).slice(0, survivor.length - config.entryLimit);
...
var SCHEMES = {
  ...
  deepseek: {
    ...
    parse: (json5) => {
      const infos = Array.isArray(json5?.balance_infos) ? json5.balance_infos : [];
      const info = infos.find((entry) => entry?.currency === "CNY") ?? infos[0];
      return {
        isAvailable: json5?.is_available === true,
  ...
  openrouter: {
    ...
    parse: (json5) => {
      const totalCredits = typeof json5?.data?.total_credits === "number" ? json5.data.total_credits : void 0;
      const totalUsage = typeof json5?.data?.total_usage === "number" ? json5.data.total_usage : void 0;
