...
// dsh-triad/src/memory/api.ts
import { URL as URL2 } from "node:url";

// dsh-triad/src/memory/types.ts
var DEFAULT_CONFIG = {
  extractEveryTurns: 1,
...
// dsh-triad/src/memory/engine/store.ts
...
function memoryHome() {
  const dshHome2 = process.env.DSH_HOME ?? join(homedir(), ".dsh");
  return join(dshHome2, "memories", "dsh-memory");
}
...
`, "utf8");
  ...
  async listDshWorkspaces() {
    const dshHome2 = process.env.DSH_HOME ?? join(homedir(), ".dsh");
    const file = join(dshHome2, "storages", "workspace.json");
...
// dsh-triad/src/memory/engine/scoring.ts
...
// dsh-triad/src/memory/engine/compile.ts
...
// dsh-triad/vendor/dsh-llm/brand.ts
...
// dsh-triad/vendor/dsh-llm/never.ts
function assertNever(value, context) {
  const rendered = JSON.stringify(value) ?? String(value);
  throw new Error(`unreachable variant${context ? ` in ${context}` : ""}: ${rendered}`);
}

// dsh-triad/vendor/dsh-llm/message.ts
import { randomUUID as randomUUID2 } from "@deepseek-ai/dsh-util-crypto";

// dsh-triad/vendor/dsh-llm/call-config.ts
...
// dsh-triad/vendor/dsh-llm/assembler.ts
...
// dsh-triad/vendor/dsh-llm/error.ts
...
// dsh-triad/src/memory/engine/embedding.ts
...
// dsh-triad/src/memory/engine/retrieval.ts
...
// dsh-triad/src/memory/engine/extract.ts
...
// dsh-triad/src/memory/engine/consolidate.ts
...
// dsh-triad/src/memory/engine/inject.ts
...
// dsh-triad/vendor/dsh-session/json.ts
...
// dsh-triad/vendor/dsh-tools/json-schema.ts
...
// dsh-triad/vendor/dsh-tools/schema.ts
...
// dsh-triad/vendor/usage-skill/skills-host.js
...
function dshRoot() {
  const dshHome2 = process.env.DSH_HOME ?? join2(homedir2(), ".dsh");
  return join2(dshHome2, "skills");
}
...
// dsh-triad/vendor/usage-skill/index.js
...
// dsh-triad/vendor/usage-skill/usage.js
...
var SCHEMES = {
  ...
  openrouter: {
    ...
    parse: (json6) => {
      const totalCredits = typeof json6?.data?.total_credits === "number" ? json6.data.total_credits : void 0;
      const totalUsage = typeof json6?.data?.total_usage === "number" ? json6.data.total_usage : void 0;
...
// dsh-triad/vendor/usage-skill/subscriptions.js
...
// dsh-triad/vendor/usage-skill/accounts.js
...
// dsh-triad/vendor/usage-skill/deepseek-billing.js
...
// dsh-triad/src/skill-toggles.ts
...
function dshRoot2() {
  const dshHome2 = process.env.DSH_HOME ?? join5(homedir5(), ".dsh");
  return join5(dshHome2, "skills");
}
...
function dshRoot3() {
  const dshHome2 = process.env.DSH_HOME ?? join6(homedir6(), ".dsh");
  return join6(dshHome2, "skills");
}
...
// dsh-triad/src/mcp-recommended.ts
...
// dsh-triad/src/mcp-status.ts
import { readFileSync as readFileSync2, writeFileSync, copyFileSync, existsSync } from "node:fs";
...
function togglePatchEntry(serverName, disabled) {
  ...
  if (entry === void 0) return { ok: false, error: `no mcp-client entry for serverName "${serverName}" in ${path}` };
